...
// TODO: implement
		 ...
		 * there already is a negative cycle => return 0 Otherwise: int minWalkLength =
		 * Integer.MAX_VALUE; // mininmaler Weg Länge for i = 0 ... n-1 for j = 0 ...
		 * n-1 if i == j continue else: minWalkLength = Math.min(minWalkLength, d[i][j]
		 * + d[j][i])
		 */
	...
	public static void initialise(int n, long[][] dist, long[][] weights, ArrayList<ArrayList<Integer>> E,
			ArrayList<ArrayList<Integer>> W) {
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (i == j)
					continue; // if it dist[i][i] = Integer.MAX_VALUE, there will always be a cycle registered
	...
	public static void FloydWarshall(int n, long[][] dist, long[][] weights) {
		for (int useUntil = 0; useUntil < n; useUntil++) {
			for (int u = 0; u < n; u++) {
				for (int v = 0; v < n; v++) {
					if (dist[u][useUntil] == Integer.MAX_VALUE || dist[useUntil][v] == Integer.MAX_VALUE) {
						continue;
					}
	...
	public static boolean hasNegativeCycle(int n, long[][] dist) {
		...
		for (int i = 0; i < n; i++) {
			if (dist[i][i] < 0) {
				return true; // hier war ein Fehler
			}
		}
		return hasNegCycle;
	}
	...
	public static long getMinCost(int n, long[][] dist) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (i == j)
					continue; // keine Schleifen, da Weg von Laenge 1, was nicht erlaubt. Selbstkanten sind in
								// unserer Definition nicht erlaubt
		...
		return (int) minCost + 1; // +1 because if not, there will simply be a walk with non-negative weight
	}
}
